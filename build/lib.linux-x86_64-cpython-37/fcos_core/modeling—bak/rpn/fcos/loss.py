...
class PrototypeComputation(object):
    ...
    def __call__(self, locations, features, targets, tgt_center, mask_t = None):
        ...
        if locations: # Sampling in the source domain
            ...
            for i in features:
                ...
                mask_tgt_list.append(F.interpolate(mask, size=[h, w], mode='bilinear'))
            # pos_level_list = []
            # neg_level_list = []
            for l, feature in enumerate(features):
                snr_map = mask_tgt_list[l].reshape(-1, 1).squeeze(1)
                ...
                if conf_pos_indx.any():
                    ...
                    neg_snr_mask.append(neg_snr_mask_tmp[neg_indx_new])
            ...
            if len(pos_points)>0:
                ...
                pos_snr_mask = torch.cat(pos_snr_mask,dim=0)
                neg_snr_mask = torch.cat(neg_snr_mask,dim=0)
                ...
                mask_tgt = torch.cat([neg_snr_mask, pos_snr_mask])
                return points, plabels, loss_weight.long(), pos_level_labels, mask_tgt
            else:
