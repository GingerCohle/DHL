...
class GModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, score_maps=None,  tgt_center=None,mask_t=None):
        '''
        ...
        '''
        if targets:
            features, feat_loss = self._forward_train(images, features, targets, score_maps, tgt_center,mask_t)
    ...
    def _forward_train(self, images, features, targets=None, score_maps=None, tgt_center=None, mask_t=None):
            ...
            nodes_1, labels_1, weights_1, src_node_level = self.graph_generator(
                self.compute_locations(features_s), features_s, targets, tgt_center, None
            )
            nodes_2, labels_2, weights_2, tgt_node_level, tgt_snr_mask = self.graph_generator(
                None, features_t, score_maps, tgt_center, mask_t
            )
            ...
            if nodes_2 is not None: # Both domains have graph nodes
                ...
                (nodes_1, nodes_2), (labels_1, labels_2), (weights_1, weights_2),(levels_1, levels_2),  tgt_snr_mask= \
                    self._forward_preprocessing_source_target((nodes_1, nodes_2), (labels_1, labels_2),(weights_1,weights_2),(src_node_level, tgt_node_level), tgt_mask=tgt_snr_mask)
                ...
                if self.with_complete_graph:
                    nodes_1, edges_1 = self._forward_intra_domain_graph(nodes_1, snr_mask=None)
                    nodes_2, edges_2 = self._forward_intra_domain_graph(nodes_2, snr_mask=tgt_snr_mask)
                ...
                nodes_1, edges_1 = self._forward_intra_domain_graph(nodes_1, snr_mask=None)
                nodes_2, edges_2 = self._forward_intra_domain_graph(nodes_2, snr_mask=None)
    ...
    def _forward_preprocessing_source_target(self, nodes, labels, weights, level_node, tgt_mask):
        ...
        tg_level_snr_mask_category_first = []
        ...
        for c in labels_exist:
            ...
            tg_snr_mask_c = tgt_mask[tg_indx]
            ...
            if sr_indx.any() and tg_indx.any(): # If the category appear in both domains, we directly collect them!
                ...
                tg_level_snr_mask_category_first.append(tg_snr_mask_c)
                ...
                tg_snr_mask_category_level_list = []
                for level in tg_level_exist:
                    ...
                    tg_snr_mask_category_level_list.append(tg_snr_mask_c[tg_lvl_idx])
                #concat level
                ...
                tg_snr_mask_category_level_list = torch.cat(tg_snr_mask_category_level_list, dim=0)
                ...
                tg_level_snr_mask_category_first.append(tg_snr_mask_category_level_list)
                ...
                for level in sr_level_exist:
                    ...
                    tg_snr_mask_category_level_list.append(torch.ones(tmp_len, dtype=torch.float).cuda().long()*1.0)
                #concat level
                sr_nodes_c_level_list = torch.cat(sr_nodes_c_level_list, dim=0)
                tg_nodes_c_level_list = torch.cat(tg_nodes_c_level_list, dim=0)
                sr_level_category_level_list = torch.cat(sr_level_category_level_list, dim=0)
        ...
        snr_mask_tg = torch.cat(tg_level_snr_mask_category_first, dim=0)

        return (nodes_sr, nodes_tg), (label_sr, label_tg), (weight_sr, weight_tg), (level_sr, level_tg), snr_mask_tg
    ...
    def _forward_intra_domain_graph(self, nodes, snr_mask):
        nodes, edges = self.intra_domain_graph(nodes, nodes, nodes, snr_mask)
        return nodes, edges
