...
def normalize(mean, std, to_bgr255, image):
    image_list = []
    for i in image:
        if to_bgr255:
            i = i[[2, 1, 0]] * 255
        image_list.append(F.normalize(i, mean=mean, std=std).unsqueeze(0))
    return torch.cat(image_list, dim=0)

def nf_transform(raw_tensor):
    tensor_list = []
    for images in raw_tensor:
        img_nf = images.cpu().permute(1, 2, 0).numpy() * 255.0
        img_nf = cv2.blur(img_nf, (5, 5))
        img_nf = img_nf * 1.0 / 255.0
        img_nf = torch.Tensor(img_nf).float().permute(2, 0, 1).cuda().unsqueeze(0)
        tensor_list.append(img_nf)
...
def foward_detector(cfg, model, images, targets=None, return_maps=True,  DA_ON=True):
    ...
    if model_fcos.training and DA_ON:
        ...
        img_nf = nf_transform(images_t.tensors.clone())
        dark = images_t.tensors.clone()
        ...
        #norm
        images_s = normalize(mean=cfg.INPUT.PIXEL_MEAN, std=cfg.INPUT.PIXEL_STD, to_bgr255=cfg.INPUT.TO_BGR255, image=images_s.tensors)
        images_t = normalize(mean=cfg.INPUT.PIXEL_MEAN, std=cfg.INPUT.PIXEL_STD, to_bgr255=cfg.INPUT.TO_BGR255, image=images_t.tensors)
        features_s = model_backbone(images_s)
        features_t = model_backbone(images_t)
        proposals, proposal_losses, _ = model_fcos(
            images_s, features_s, targets=targets)

        if with_middle_head:
            score_maps, zeros, ctr = model_fcos(images_t, features_t, targets=None)
            ...
            (features_s, features_t), middle_head_loss = model_middle_head(images, (features_s,features_t), targets=targets, score_maps=score_maps, tgt_center=ctr, mask_t = mask)
